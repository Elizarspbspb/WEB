...
<head>
    ...
    <title>Апартаменты_admin_pokupatel</title>
</head>
<body>   
    <div class=" container_header"> 
    <h3 class="section_suptitle">Все наши Клиенты в агентстве</h3> 
    <form action="" method="post">
    <?
			$sql = 'select покупатель.idПокупатель as №, покупатель.Фамилия, покупатель.Имя, покупатель.Отчество, покупатель.`Паспортные данные` as Паспорт, покупатель.Телефон, покупатель.Email from Покупатель';
...
<?
    if ( $_POST['del'] )
    {
        ...
        foreach($array_apart as $state => $mass){
			$sql = "delete from покупатель where idПокупатель = :idT";
            $params = [
                'idT' => $mass
            ];
            $statement = $connection->prepare($sql);
...
?>
	...
	<h3 class="section_suptitle"> Обновить данные Покупателя </h3>
        <form action='updateP.php' method="post" class="myForm">
            <p id="error"></p>
            ...
            <br>
			<input  required  name="pasport" class="input" type="text" placeholder="Паспортные данные:">
			<br>
			<input  required  name="number" class="input" type="text" placeholder="Телефон:">
            <br>
